...
> 對應課綱 Segment 4，40 分鐘。
>
> 前三節教的是「原理能跑」；本節把 mini-project 放到**真實世界**，看看要面對哪些牆：**錢、模型選擇、品質陷阱、規模**。內容大量引用 2026-04-24 的 Claude vs Gemma 4 實驗數據（`mini-project/docs/benchmarks/2026-04-24-claude-vs-gemma4.md`）。
...
```
Segment 1-3 教學目標:  「原理上能跑起來」
Segment 4 教學目標:    「放進生產會遇到什麼」
Segment 5 教學目標:    「你親手跑一個最小版本」
```

常見誤區：課堂 demo 成功 ≠ production ready。中間隔著四件事：

1. **💰 錢** — API 費用、GPU 電費、運維人力
2. **🧠 模型選擇** — 不同任務用不同模型，不是一個打天下
3. **🎯 品質陷阱** — 邊界防禦、docstring 設計、context 管理
4. **📐 規模** — 3 個工具 vs 239 個工具是**質變**，不是量變
...
## §4 從 mini-project 到 production 的 scale 跳（10 分鐘）
...
**本節結束 → Segment 5 動手指引（請翻 `05-hands-on-lab.md`）**
