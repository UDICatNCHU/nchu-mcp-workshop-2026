...
> 你剛在 Segment 4 親手跑完 mini-project。這一份把「**你做的東西放到真實世界會遇到什麼**」整理成四大支柱，趁此刻最有感的時候一次點出。
>
> 文中的數據都來自 2026-04-24 的 Claude vs Gemma 4 實驗（見 [benchmark 報告](./mini-project/docs/benchmarks/2026-04-24-claude-vs-gemma4.md)）。

---

## 怎麼讀這份

四大支柱不用一次讀完 —— 挑跟你情境最相關的先看：

- 你是**一般老師 / 研究生** → 先看 **§1 Scale**（從「你剛做的 3 工具」到「239 工具怎麼辦」），最有感
- 你**正要把這套導入課程** → 先看 **§4 成本**（把真實帳單算清楚）
- 你**想用在研究上** → 先看 **§3 模型選擇**（有 benchmark 數據佐證）
- 你**寫過 agent** → 先看 **§2 品質陷阱**（docstring、邊界防禦、錯誤當資料）
